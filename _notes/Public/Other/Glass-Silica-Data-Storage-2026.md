...
### Voxel คืออะไร? (เข้าใจใน 30 วินาที)
...
> **ทำไมถึงเก็บได้หลาย level?** เพราะ voxel ไม่ได้เก็บแค่ "มี/ไม่มี" แต่เก็บ **สมบัติทางแสง** ได้หลายค่า — แต่ละค่าสามารถแบ่งเป็น multiple levels ได้:

### สมบัติทางแสงที่ voxel ใช้เก็บข้อมูล

**1. มุม Polarization (Slow Axis Orientation)** 🔄

แสงเป็นคลื่นแม่เหล็กไฟฟ้า — สั่นได้หลายทิศทาง:
```
...
- Nanograting หมุน **มุมของแสง** ที่ผ่านมัน → แต่ละมุม = ค่าต่างกัน
...
Retardance = แสงส่อผ่าน voxel → ถูก **หน่วง/ชะลอ** ไม่เท่ากันในแต่ละทิศ:
```
...
- Nanograting ทำให้แสงเดินทิศหนึ่ง **ช้ากว่า** อีกทิศ → เรียกว่า **birefringence**
- ยิ่ง nanograting หนาแน่น → retardance สูง → แสงถูกหน่วงมาก
- เลเซอร์ควบคุม **intensity (พลังงาน)** → ควบคุมความหนาแน่นของ nanograting → ควบคุม retardance
...
Refractive index = ตัวเลขบอกว่า **แสงเดินช้าลงแค่ไหน** เมื่อผ่านวัสดุ:
```
อากาศ (n=1.0):     แก้ว (n=1.5):
แสง →  ████████     แสง →  ████████
       เร็ว (300K km/s)    ช้าลง 33% (200K km/s)
       
Voxel: เลเซอร์เปลี่ยน n ตรงจุดนั้น:
แสง →  [n=1.501] [n=1.505] [n=1.510]
         level 1    level 2    level 3
```

- Femtosecond laser pulse เปลี่ยน **ความหนาแน่นของแก้ว** ตรงจุด voxel แบบ isotropic (เท่ากันทุกทิศ)
- ใช้กับ **Phase Voxels** — สำหรับ Borosilicate (Pyrex)
...
**สรุป: แต่ละสมบัติเก็บข้อมูลยังไง**

| สมบัติ | วัดอะไร | ควบคุมยังไง | ใช้ใน voxel ประเภทไหน |
|:------|:--------|:-----------|:---------------------|
| **Polarization angle** | มุมที่แสงถูกหมุน | ทิศทาง polarization ของเลเซอร์ | Birefringent (Fused Silica) |
| **Retardance** | แสงถูกหน่วงแค่ไหน | พลังงานเลเซอร์ → ความหนาแน่น nanograting | Birefringent (Fused Silica) |
| **Refractive index** | แสงเดินช้าลงแค่ไหน | พลังงานเลเซอร์ → ความหนาแน่นแก้วตรงจุด | Phase (Borosilicate) |

**ขนาดของ voxel vs สิ่งอื่น:**
