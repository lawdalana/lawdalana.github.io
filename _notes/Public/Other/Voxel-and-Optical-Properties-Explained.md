...
> อธิบาย **voxel** และ **สมบัติทางแสง** ที่ทำให้ Glass/Silica Data Storage เก็บข้อมูลได้หลาย bits ต่อ voxel ทั้ง polarization, retardance และ refractive index ด้วยภาษาที่เข้าใจง่ายและภาพประกอบ
>
...
| **Nanograting** | ตะแกรงนาโน / โครงสร้างแผ่นเรียงระดับนาโน | แผ่นบาง ๆ เรียงกันเป็นระเบียบระดับนาโนเมตร (หนา ~20nm) ที่เลเซอร์สร้างขึ้นในแก้ว |
| **Nanovoid** | ช่องว่างระดับนาโน | ช่องว่างเล็ก ๆ ระดับนาโนที่เกิดจากการยิงเลเซอร์พลังงานสูงเข้าไปในแก้ว |
...
| **Birefringence** | ไบร์เฟรงเจนซ์ / สมบัติหักเหสองเงา | สมบัติที่วัสดุมีดัชนีหักเหต่างกันตามทิศทาง ทำให้แสงเคลื่อนที่ด้วยความเร็วต่างกัน |
| **Refractive Index (n)** | ดัชนีหักเห | ตัวเลขบอกว่าแสงเคลื่อนที่ช้าลงแค่ไหนเมื่อผ่านวัสดุ (อากาศ=1.0, แก้ว≈1.5) |
| **Femtosecond Laser** | เลเซอร์เฟมโตวินาที | เลเซอร์ที่ยิงพัลส์สั้นมาก (10⁻¹⁵ วินาที) ใช้สร้าง voxel ในแก้ว |
| **Fused Silica** | แก้วซิลิกา | แก้วบริสุทธิ์ชนิดหนึ่ง (SiO₂) ที่ใช้ใน Project Silica |
| **Borosilicate** | โบโรซิลิเคต | แก้วชนิดหนึ่ง (เช่น Pyrex) ถูกกว่า Fused Silica แต่เก็บข้อมูลได้น้อยกว่า |
| **Isotropic** | ไอโซทรอปิก / สมบัติเหมือนกันทุกทิศทาง | สมบัติไม่เปลี่ยนตามทิศทาง |
| **Anisotropic** | แอนไอโซทรอปิก / สมบัติขึ้นกับทิศทาง | สมบัติต่างกันตามทิศทาง (เช่น nanograting ทำให้แก้วเป็น anisotropic) |
...
4. **การเปลี่ยนแปลงถาวร** — โครงสร้างที่เปลี่ยนไปคงอยู่ได้โดยไม่ต้องใช้ไฟเลี้ยง (อายุ 10,000+ ปี)
...
<a id="level-คืออะไร-ทำไมเยอะได้เยอะ-bits"></a>

## "Level" คืออะไร และเกี่ยวกับจำนวน bits อย่างไร?

### Level คือค่าที่แตกต่างกันได้ของสมบัติทางแสง

ในที่นี้ "level" หมายถึง **ค่าของสมบัติทางแสงที่เครื่องอ่านแยกแยะได้** ไม่ใช่ชั้นที่วาง voxel ในแก้ว

**เปรียบเทียบให้เห็นภาพ:**

```
สวิตช์เปิด-ปิด (แยกแค่ 2 level):
  ปิด → OFF → เก็บ "0"
  เปิด → ON  → เก็บ "1"
  → 2 states = 1 bit

สวิตช์หมุนที่เลือกได้ 8 ตำแหน่ง (8 levels):
  ตำแหน่ง 0 → เก็บ "000"
  ตำแหน่ง 1 → เก็บ "001"
  ...
  ตำแหน่ง 7 → เก็บ "111"
...
- **100 levels** มีความจุข้อมูลเทียบเท่า log₂(100) ≈ **6.644 bits**
...
ถ้ามี 100 level หมายความว่า:
  ...
  คิดเป็นความจุข้อมูลเทียบเท่า 6.644 bits ต่อ voxel
```
...
Voxel ไม่ได้เก็บแค่ "มี/ไม่มี" (เหมือน CD ที่มีแค่ pit/land) แต่ใช้ **สมบัติทางแสง** เข้ารหัสข้อมูล โดยแต่ละสมบัติแบ่งเป็นหลายระดับได้:
...
> 💡 **เปรียบเทียบกับสี:** CD เหมือนดินสอดำที่เลือกได้ว่าจะระบายหรือเว้นว่าง ส่วน Glass Storage เหมือนกล่องสีเทียน 100 สี โดยแต่ละสีแทนข้อมูลคนละค่า
...
Nanograting ทำให้สมบัติของแก้วขึ้นกับทิศทาง (**anisotropic**) ลองนึกถึงไม้ไผ่ที่เรียงขนานกัน:
...
> **ความสัมพันธ์กับ Birefringence:** Retardance เกิดจาก **birefringence (ไบร์เฟรงเจนซ์)** หรือสมบัติของวัสดุที่มี refractive index ต่างกันตามทิศทาง แสงจึงเคลื่อนที่ด้วยความเร็วต่างกันและเกิดความหน่วงต่างกัน

---

### 3. Refractive Index Change (การเปลี่ยนดัชนีหักเห)

**Refractive Index (ดัชนีหักเห, n) คือ** ตัวเลขบอกว่า **แสงเคลื่อนที่ช้าลงแค่ไหน** เมื่อผ่านวัสดุ
...
> 💡 **สรุป:** การเลือกวัสดุต้องพิจารณาทั้งช่วงที่ปรับค่า n ได้ ความเสถียร และราคา วัสดุที่ปรับ n ได้มาก เช่น chalcogenide หรือ LiNbO₃ อาจไม่เสถียรหรือมีราคาแพง Glass Storage จึงเลือก Fused Silica/Borosilicate เพราะ **เสถียร 10,000+ ปี** แม้ Δn จะน้อย
...
> 💡 **เกร็ดน่ารู้:** Nanograting เกิดจากปรากฏการณ์ **self-organization** เลเซอร์ไม่ได้ "แกะสลัก" แต่ละแผ่นโดยตรง แต่พลังงานเลเซอร์ทำให้อะตอมในแก้วจัดเรียงตัวเป็นระเบียบ (self-assembly) คล้ายคลื่นทะเลที่ทำให้ทรายเรียงเป็นลาย
...
| **Refractive index** | ดัชนีหักเห | แสงเคลื่อนที่ช้าลงแค่ไหน | พลังงานเลเซอร์ → ความหนาแน่นแก้วตรงจุด | Phase (Borosilicate) | ปานกลาง |

**Birefringent Voxels** ใช้ polarization + retardance → เก็บได้หลาย bits (สูงสุด ~6.6 bits/voxel) แต่ต้องใช้ Fused Silica แพง

**Phase Voxels** ใช้การเปลี่ยน refractive index จึงเก็บข้อมูลได้น้อยกว่า แต่ใช้กับ Borosilicate (Pyrex) ที่ถูกกว่ามากได้ การเลือกวัสดุจึงต้อง **ชั่งน้ำหนักระหว่างต้นทุนกับความจุ**
...
> 💡 **อีกตัวอย่าง:** ถ้า CD เปรียบเหมือนเหรียญ (หัว/ก้อย = 2 ค่า = 1 bit) voxel ที่มี 100 levels ก็เหมือนลูกเต๋า 100 หน้า (100 ค่า ≈ 6.6 bits) หลักการเลือกสถานะคล้ายกัน แต่เก็บข้อมูลได้มากกว่า 6 เท่า
